...
const getQuests = async () => {
	const res = await axios(
		"https://raw.githubusercontent.com/rajinzajin/ffxiv-data-csv/master/csv/ClassJob.csv"
	);
	return res.data;
};
	...
	getQuests().then((quest_csv) => {
		...
		csvStream
			...
			.on("data", (data) => {
				...
				const ClassJobCategory = data["ClassJobCategory"]
				result.push({id, Name, Abbreviation, ClassJobCategory});
			})
			.on("end", () => {
				fs.writeFileSync("./data/class_job.json", JSON.stringify(result));
